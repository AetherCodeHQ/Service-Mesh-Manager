...
> Service-Mesh-Manager - Open source tool by AetherCodeHQ

`cli` `devops` `golang` `microservices` `service-mesh`

## What is it?

**Service Mesh Manager** is a devops tool designed for developers who need fast, reliable, and offline-capable tools. Built with Go for maximum performance and minimal resource usage.
...
```bash
# Basic usage
./Service-Mesh-Manager --path ./target

# With options
./Service-Mesh-Manager --path ./target --format json --output report.json

# Verbose mode
./Service-Mesh-Manager --path ./target --verbose
```
...
### Basic scan
```bash
./Service-Mesh-Manager --path ./my-project
```

### JSON report
```bash
./Service-Mesh-Manager --path ./my-project --format json --output report.json
```

### CI/CD integration
```yaml
# .github/workflows/scan.yml
- name: Run Service Mesh Manager
  run: ./Service-Mesh-Manager --path . --format json --output report.json
```
